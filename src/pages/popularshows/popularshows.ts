...
/**
 * Generated class for the PopularshowsPage page.
 *
...
@IonicPage()
@Component({
  selector: "page-popularshows",
  templateUrl: "popularshows.html"
})
export class PopularshowsPage {
  ...
  loadpoptvShows(infiniteScroll?) {
    this.movieService
      .gettvShows(this.page)
      .then(tvshows => {
        this.popseries = this.popseries.concat(tvshows["results"]);
      })
      .catch(err => {
  ...
  loadMoreMovies(infiniteScroll) {
    this.page++;
    this.loadpoptvShows(infiniteScroll);
